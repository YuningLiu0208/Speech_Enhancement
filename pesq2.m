...
%   P.862 [1].
%
%   Usage:  [pesq_val] = pesq(reference, altered, fs)
%           
...
% ----------------------------------------------------------------------
% Modified to add composite folder to MATLAB path for dependencies
% Date: June 2025

% Add composite folder to MATLAB path
% Determine the path to the composite folder relative to this file
composite_path = fullfile(fileparts(mfilename('fullpath')),  'composite');
if exist(composite_path, 'dir')
    addpath(composite_path);
else
    error('Composite folder not found at %s. Please ensure composite.zip is extracted to the project directory.', composite_path);
end
...
Window = 0.5 * (1.0 - cos((6.28318530717959 * count) / Align_Nfft));
...
pesq_val = pesq_psychoacoustic_model(ref_data, ref_Nsamples, deg_data, deg_Nsamples);

% Remove composite path after execution to avoid path clutter
rmpath(composite_path);